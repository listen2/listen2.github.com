...
function sort_and_show(sfunc) {
	...
	var s = "<table><tr><td onclick='sort_and_show(sort_sim)'>similarity</td><td>flair text</td><td>user</td><td>last /r/buffy post</td><td onclick='sort_and_show(sort_freq_r)'>avg. post frequency</td></tr>";
	for (x in r) {
		...
		if (r[x].num_posts === 0) {
			...
			s += "<td style='background:#99f'>error checking comments</td><td>";
	  		...
	  		s += "<td style='background:#" + get_color(elapsed(r[x].last_post) / 7776000, 0.6) + "'>" + nowago(r[x].last_post) + " ago</td><td>every " + timeago(r[x].freq) /*+ " (over the last " + nowago(r[x].first_post) + ")"*/;
		}
		s += "</td></tr>";
	}
	s += "</table>";
